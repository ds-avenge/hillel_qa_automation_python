...
# ------
minimum_year, minimum_engine_volume, maximum_price = search_criteria
matching_cars = []

for car_name, car_info in car_data.items():
    color, year, engine_volume, car_type, price = car_info

    if (
        year >= minimum_year
        and engine_volume >= minimum_engine_volume
        and price <= maximum_price
    ):
        matching_cars.append((car_name, car_info))

matching_cars.sort(key=lambda item: item[1][4])

print("Here are the 5 cheapest cars that match the search criteria:")

for number, (car_name, car_info) in enumerate(matching_cars[:5], start=1):
    color, year, engine_volume, car_type, price = car_info

    print(f"\nCar #{number}")
    print(f"Brand: {car_name}")
    print(f"Color: {color}")
    print(f"Year: {year}")
    print(f"Engine volume: {engine_volume} L")
